...
# install.packages('circular') # uncomment it, if you did not install this package
library(circular)
...
getMixtmodel <-function(data,svalues) {   
  ...
  burnin<-500
  set.seed(1234)
  propsd <- svalues*.05   # tuning parameters
  lb <- c(0,4)
  ub <- c(1,360)
  ...
  for (i in c(2:dim(chain)[1])) { 
    ...
    doitagain <- TRUE
    while (doitagain) {
      propl <- cur + rnorm(2,0,propsd)    # two proposals (a vector)
      doitagain <- any(propl<lb) || any(propl>ub)  # upper bound, lower bound
    }
  ...
  td<-c(-180:180)
  pred<- (1-finparm[1])*dvonmises(mkcirc(td),mkcirc(0),sd2k(finparm[2])) + 
